"use server";

import { env } from "env.mjs";
import { type z } from "zod";
import {
  updateReviewResponseSchema,
  type updateReviewSchema,
} from "@/types/review-service";
import { getServerSession } from "@/lib/auth";
import { parseType } from "@/lib/utils";

export async function updateReview(
  id: string,
  body: z.infer<typeof updateReviewSchema>
): Promise<z.infer<typeof updateReviewResponseSchema>> {
  const session = await getServerSession(); // this will retrieve new access token if it's expired
  if (!session) {
    throw new Error("No session");
  }

  const response = await fetch(`${env.API_URL}/v1/reviews/${id}`, {
    method: "PUT",
    body: JSON.stringify({
      ...body,
      accessToken: body.accessToken ?? session.accessToken,
    }),
  });

  return parseType(updateReviewResponseSchema, await response.json());
}
